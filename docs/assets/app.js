...
drawModelInputs(d);drawStationGuidance(d,city);
const sources=Object.entries(d.sources||{});if(d.weathernext?.hourly?.length)sources.push(['WeatherNext 2 (research)',d.weathernext]);const series=sources.map(([name,v])=>({name,points:v.hourly||[]}));$('hourly-chart').innerHTML=chart(series,d.observed?.hourly||[],city.reporting_tz||city.tz);$('chart-note').textContent=`Model member medians; dark line shows station observations. Times use ${city.reporting_tz||city.tz} reporting time.`;
