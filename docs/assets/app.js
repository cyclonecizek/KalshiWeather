...
const sources=Object.entries(d.sources||{});if(d.weathernext?.hourly?.length)sources.push(['WeatherNext 2 (research)',d.weathernext]);const series=sources.map(([name,v])=>({name,points:v.hourly||[]}));const mos=mosChartSeries(d,city);if(mos)series.push(mos);$('hourly-chart').innerHTML=chart(series,d.observed?.hourly||[],city.reporting_tz||city.tz);$('chart-note').textContent=`Model member medians; dark line shows station observations. Times use ${city.reporting_tz||city.tz} reporting time. ${mos?'MOS dots show native TMP samples without interpolation; the explicit N/X daytime maximum remains in the MOS panel.':'MOS samples are unavailable for this station and reporting day; see the MOS panel for status.'}`;
$('chart-legend').innerHTML=series.map((s,i)=>`<span><i class="swatch" style="background:${s.color||colors[i%colors.length]}"></i>${esc(s.name)}</span>`).join('')+'<span><i class="swatch" style="background:#1f3231"></i>Observed</span>';
