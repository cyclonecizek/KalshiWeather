...
const colors=['#6bd8ee','#c4a3ff','#74dfbd','#f0c37b','#8daeff','#f89cad'];
...
function mosChartSeries(day,city) {
 ...
 return {name:`GFS MOS TMP (native samples${stale?'; stale':''})`,points,color:'#f6b57b',markers:true};
}
function mosChartExtrema(day,city) {
 ...
 return extrema.filter(e=>['maximum','minimum'].includes(e.kind)&&Number.isFinite(e.temperature_f)&&Date.parse(e.period_end)>Date.parse(e.period_start))
  .map(e=>({...e,start:Math.max(Date.parse(e.period_start),Date.parse(day.window_start)),end:Math.min(Date.parse(e.period_end),Date.parse(day.window_end)),label:e.kind==='maximum'?'X':'N',color:e.kind==='maximum'?'#ff9c9c':'#8bbdff',stale:mos.status==='stale'||M.age(mos.issued_at)>720}))
...
for(let i=0;i<=4;i++){const v=ymin+(ymax-ymin)*i/4;s+=`<line x1="${L}" x2="${W-R}" y1="${y(v)}" y2="${y(v)}" stroke="#29394f"/><text x="${L-8}" y="${y(v)+4}" text-anchor="end" fill="#a0afc3" font-size="11">${v.toFixed(0)}°</text>`;}
for(let i=0;i<=6;i++){const t=new Date(xmin+(xmax-xmin)*i/6),xx=L+(W-L-R)*i/6;const label=t.toLocaleTimeString('en-US',{hour:'numeric',timeZone:tz});s+=`<text x="${xx}" y="${H-14}" text-anchor="middle" font-size="11" fill="#a0afc3">${esc(label)}</text>`;}
series.forEach((a,i)=>{const color=a.color||colors[i%colors.length];if(a.markers){s+=a.points.map(p=>`<circle cx="${x(p.time)}" cy="${y(p.median)}" r="4" fill="${color}" stroke="#111c2c" stroke-width="1"><title>${esc(a.name)}: ${esc(num(p.median))}°F at ${esc(new Date(p.time).toLocaleString('en-US',{timeZone:tz}))}</title></circle>`).join('');}else{s+=`<polyline points="${a.points.filter(p=>Number.isFinite(p.median)).map(p=>`${x(p.time)},${y(p.median)}`).join(' ')}" fill="none" stroke="${color}" stroke-width="2"/>`;}});
s+=`<polyline points="${observed.map(p=>`${x(p.time)},${y(p.temperature_f)}`).join(' ')}" fill="none" stroke="#edf5ff" stroke-width="3"/>`;
for(const e of extrema){const a=x(new Date(e.start).toISOString()),b=x(new Date(e.end).toISOString()),yy=y(e.temperature_f);s+=`<g><title>MOS ${esc(e.label)} ${num(e.temperature_f)}°F${e.stale?' (stale)':''}: ${esc(new Date(e.period_start).toLocaleString('en-US',{timeZone:tz}))} to ${esc(new Date(e.period_end).toLocaleString('en-US',{timeZone:tz}))}. Period extremum, not an hourly temperature.</title><line x1="${a}" x2="${b}" y1="${yy}" y2="${yy}" stroke="${e.color}" stroke-width="2" stroke-dasharray="6 4"/><text x="${(a+b)/2}" y="${yy-7}" text-anchor="middle" fill="${e.color}" stroke="#111c2c" stroke-width="3" paint-order="stroke" font-size="12">${esc(e.label)} ${num(e.temperature_f,0)}°${e.stale?' (stale)':''}</text></g>`;}
return s+'</svg>';}
...
$('reliability').innerHTML=bins.length?`<svg viewBox="0 0 420 290" role="img" aria-label="Predicted probability versus observed frequency"><line x1="45" y1="245" x2="385" y2="20" stroke="#788da8" stroke-dasharray="5 5"/><line x1="45" y1="245" x2="385" y2="245" stroke="#637994"/><line x1="45" y1="245" x2="45" y2="20" stroke="#637994"/>${[0,.25,.5,.75,1].map(p=>`<text x="${45+p*340}" y="263" text-anchor="middle" font-size="10">${pct(p)}</text><text x="35" y="${249-p*225}" text-anchor="end" font-size="10">${pct(p)}</text>`).join('')}${bins.map(b=>`<circle cx="${45+b.p*340}" cy="${245-b.y*225}" r="${Math.min(12,4+Math.sqrt(b.n))}" fill="#6bd8ee"><title>${pct(b.p)} forecast, ${pct(b.y)} observed, n=${b.n}</title></circle>`).join('')}<text x="210" y="286" text-anchor="middle" font-size="11">Forecast probability</text></svg>`:empty('Calibration points appear after settlement.');
drawModelResearch();}
