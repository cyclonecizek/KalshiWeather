...
def parse(text, station, product):
    ...
    for line in block.splitlines():
        key = line[:3].strip()
        if key in ('HR', 'UTC', 'TMP', 'DPT', 'CLD', 'WDR', 'WSP', 'PPO', 'P01', 'P06', 'P12'):
            lines[key] = line
    ...
    if not any(p['temperature_f'] is not None for p in points):
        raise ValueError('No valid temperatures')
    return {'station': station, 'product': product, 'issued_at': run.isoformat(),
            'points': points, 'precipitation': periods, 'blend_weight': 0,
...
def for_window(source, start, end):
    result = {k: v for k, v in source.items() if k not in ('points', 'precipitation')}
